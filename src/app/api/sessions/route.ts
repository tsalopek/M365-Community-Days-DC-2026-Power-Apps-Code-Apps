...
export async function GET(request: Request) {
    try {
        ...
        if (!listUrl) {
            return Response.json({ error: 'SharePoint sessions URL is not configured.' }, { status: 500 });
        }
        ...
        if (!response.ok) {
            // SharePoint returns 401 if user lacks permissions, 403 for forbidden
            if (response.status === 401) {
                return Response.json(
                    { error: 'Your account does not have permission to access the sessions list.' },
                    { status: 401 }
                );
            }
        ...
        return Response.json(
            {
                error:
                    error instanceof Error
                        ? error.message
                        : 'Failed to fetch sessions from SharePoint',
            },
            { status: 500 }
        );
    }
