    ...
    try {
        ...
        if (!session || !session.user) {
            return Response.json(
                { error: 'Unauthorized. Please sign in to view sponsors.' },
                { status: 401 }
            );
        }
        ...
        if (!listUrl) {
            return Response.json({ error: 'SharePoint sponsors URL is not configured.' }, { status: 500 });
        }
        ...
        if (!response.ok) {
            if (response.status === 401) {
                return Response.json(
                    { error: 'Your account does not have permission to access the sponsors list.' },
                    { status: 401 }
                );
            }
        ...
        return Response.json(
            {
                error:
                    error instanceof Error
                        ? error.message
                        : 'Failed to fetch sponsors from SharePoint',
            },
            { status: 500 }
        );
    }
